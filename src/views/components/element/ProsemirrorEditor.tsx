...
const ProsemirrorEditor = forwardRef<ProsemirrorEditorRef, ProsemirrorEditorProps>(({
  ...
  const handleInput = useCallback(
    debounce((isHanldeHistory = false) => {
  ...
  const handleFocus = useCallback(() => {
    ...
    onFocus?.()
  }, [ctrlOrShiftKeyActive, activeElementIdList, setDisableHotkeysState, onFocus])
  ...
  const handleKeydown = useCallback((view: EditorView, e: KeyboardEvent) => {
  ...
  useEffect(() => {
    ...
    const view = initProsemirrorEditor(
  ...
  const execCommand = useCallback(({ target, action }: RichTextCommand) => {
  ...
  return (
    <div
      className={clsx('prosemirror-editor', { 'format-painter': textFormatPainter })}
      ref={editorViewRef}
