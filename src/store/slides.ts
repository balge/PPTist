...
const updateComputed = (state: Partial<SlidesState>): Partial<SlidesState> => {
  ...
  const currentSlideAnimations: PPTAnimation[] = []
  const formatedAnimations: FormatedAnimation[] = []

  if (currentSlide && currentSlide.animations) {
    const els = currentSlide.elements
    const elIds = els.map((el) => el.id)
    const animations = currentSlide.animations.filter((animation) =>
      elIds.includes(animation.elId)
    )
    currentSlideAnimations.push(...animations)
  ...
  return { currentSlide, currentSlideAnimations, formatedAnimations }
}
...
export const useSlidesStore = create<SlidesState>((set, get) => ({
  ...
  setViewportRatio: (viewportRatio: number) =>
    set({ viewportRatio: viewportRatio }),
  ...
  updateElement: (data: UpdateElementData) =>
    set((state) => {
      ...
      newSlides[slideIndex] = {
        ...slide,
        elements: newElements as PPTElement[],
      }
